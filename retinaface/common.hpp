...
static inline cv::Rect get_rect_adapt_landmark(cv::Mat &img, int input_w, int input_h, float bbox[4], float lmk[RetinaFace::NUM_LANDMARKS])
{
    int l, r, t, b;
    float r_w = input_w / (img.cols * 1.0);
    float r_h = input_h / (img.rows * 1.0);
    if (r_h > r_w)
    {
        ...
        for (int i = 0; i < RetinaFace::NUM_LANDMARKS; i += 2)
        {
            lmk[i] /= r_w;
...
static bool cmp(const RetinaFace::Detection &a, const RetinaFace::Detection &b)
{
    return a.class_confidence > b.class_confidence;
}

static inline void nms(std::vector<RetinaFace::Detection> &res, float *output, float nms_thresh = 0.4)
{
    std::vector<RetinaFace::Detection> dets;
    for (int i = 0; i < output[0]; i++)
    {
        if (output[RetinaFace::ENTRY_LENGTH * i + 1 + 4] <= 0.1)
            continue;
        RetinaFace::Detection det;
        memcpy(&det, &output[RetinaFace::ENTRY_LENGTH * i + 1], sizeof(RetinaFace::Detection));
        dets.push_back(det);
    }
