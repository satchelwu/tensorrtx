...
static const int INPUT_H = RetinaFace::INPUT_H;  // H, W must be able to  be divided by 32.
static const int INPUT_W = RetinaFace::INPUT_W;;
static const int OUTPUT_SIZE = (INPUT_H / 8 * INPUT_W / 8 + INPUT_H / 16 * INPUT_W / 16 + INPUT_H / 32 * INPUT_W / 32) * 2  * RetinaFace::ENTRY_LENGTH + 1;
...
ICudaEngine* createEngine(unsigned int maxBatchSize, IBuilder* builder, IBuilderConfig* config, DataType dt) {
    ...
    auto lmk_head1 = network->addConvolutionNd(*ssh1->getOutput(0), 2 * RetinaFace::NUM_LANDMARKS, DimsHW{1, 1}, weightMap["LandmarkHead.0.conv1x1.weight"], weightMap["LandmarkHead.0.conv1x1.bias"]);
    auto lmk_head2 = network->addConvolutionNd(*ssh2->getOutput(0), 2 * RetinaFace::NUM_LANDMARKS, DimsHW{1, 1}, weightMap["LandmarkHead.1.conv1x1.weight"], weightMap["LandmarkHead.1.conv1x1.bias"]);
    auto lmk_head3 = network->addConvolutionNd(*ssh3->getOutput(0), 2 * RetinaFace::NUM_LANDMARKS, DimsHW{1, 1}, weightMap["LandmarkHead.2.conv1x1.weight"], weightMap["LandmarkHead.2.conv1x1.bias"]);
    ...
    std::vector<int> pfc_data = {INPUT_W, INPUT_H, RetinaFace::NUM_LANDMARKS};
    pfc.nbFields = 3;
    pfc.fields = new PluginField[pfc.nbFields];
    
    PluginField* p_pf = const_cast<PluginField*>(pfc.fields);
    p_pf[0].data = &pfc_data[0];
    p_pf[0].type = nvinfer1::PluginFieldType::kINT32;
    p_pf[0].length = sizeof(int);
    p_pf[0].name = "net_width";

    p_pf[1].data = &pfc_data[1];
    p_pf[1].type = nvinfer1::PluginFieldType::kINT32;
    p_pf[1].length = sizeof(int);
    p_pf[1].name = "net_height";

    p_pf[2].data = &pfc_data[2];
    p_pf[2].type = nvinfer1::PluginFieldType::kINT32;
    p_pf[2].length = sizeof(int);
    p_pf[2].name = "num_landmarks";
...
void doInference(IExecutionContext& context, float* input, float* output, int batchSize) {
    ...
    std::cout << "trt infer done" << std::endl;
...
int main(int argc, char** argv) {
    ...
    int num_binds = engine->getNbBindings();
    for(int bind_index = 0; bind_index < num_binds; ++bind_index)
    {
        std::string name = engine->getBindingName(bind_index);
        std::cout << "name: " << name;
        Dims dim = engine->getBindingDimensions(bind_index);
        std::cout << " shape: (";
        for(int dim_index = 0; dim_index < dim.nbDims; ++dim_index)
        {
            std::cout << dim.d[dim_index] << ", ";
        }
        std::cout << ")" << std::endl;
    }
    //ICudaEngine* engine = runtime->deserializeCudaEngine(trtModelStream, size, nullptr);
    assert(engine != nullptr);
    std::cout << "deserilize done..." << std::endl;;
    ...
    for (int b = 0; b < BATCH_SIZE; b++) {
        std::vector<RetinaFace::Detection> res;
        ...
        for (size_t j = 0; j < res.size(); j++) {
            ...
            for (int k = 0; k < 8; k += 2) {
                cv::circle(tmp, cv::Point(res[j].landmark[k], res[j].landmark[k + 1]), 5, {0, 0, 255}, 4);
            }
        }
